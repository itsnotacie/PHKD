...
class LocalizationNet(nn.Module):
    ...
    def World2GrdImgPixCoordinates(self, ori_shift_u, ori_shift_v, ori_heading, XYZ_1, ori_camera_k, grd_H, grd_W, ori_grdH,
        ...
        # with torch.no_grad():
        valid_area = torch.greater(uv1_last, torch.ones_like(uv1[:, :, :, 2:]) * 1e-6) * \
    ...
    def project_grd_to_map(self, grd_f, grd_c, shift_u, shift_v, heading, camera_k, satmap_sidelength, ori_grdH,
        ...
        uv, valid_area = self.World2GrdImgPixCoordinates(shift_u, shift_v, heading, XYZ_1, camera_k, H, W, ori_grdH, ori_grdW)  # [B, S, E, H, W,2]
        # [B, H, W, 2], [B, H, W, 1]

        grd_f_trans, new_jac = grid_sample(grd_f, uv, None)
        # [B,C,sidelength,sidelength], [3, B, C, sidelength, sidelength]
        grd_f_trans = grd_f_trans * valid_area[:, None, :, :, 0]
        if grd_c is not None:
            grd_c_trans, _ = grid_sample(grd_c, uv)
            grd_c_trans = grd_c_trans * valid_area[:, None, :, :, 0]
        else:
            grd_c_trans = None


        return grd_f_trans, grd_c_trans, uv, valid_area

    def forward(self, sat_feat_list, grd_feat_list, left_camera_k):
        '''
        rot_corr
        Args:
        ...
        for _, level in enumerate(self.levels):
            ...
            A = sat_feat.shape[-1]
            grd_feat_proj, grd_conf_proj, grd_uv, valid_area = self.project_grd_to_map(
        ...
        return sat_feat_dict, g2s_feat_dict

    def calc_corr_for_train(self, sat_feat_dict, bev_feat_dict, batch_wise=False):
        corr_maps = {}
    ...
    def calc_corr_for_val(self, sat_feat_dict, bev_feat_dict):
        level = self.levels[-1]
