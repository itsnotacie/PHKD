...
def get_same_padding_conv2d(circular, image_size=None):
    ...
    Returns:
        Conv2dDynamicSamePadding or Conv2dStaticSamePadding, or circular padding version.
    """
    if image_size is None:
        if circular:
            return Conv2dDynamicCircularPadding
        else:
            return Conv2dDynamicSamePadding
    else:
        if circular:
            return partial(Conv2dStaticCircularPadding, image_size=image_size)
        else:
...
class Conv2dDynamicCircularPadding(nn.Conv2d):
    """2D Convolutions like TensorFlow, for a dynamic image size.
       The padding is operated in forward function by calculating dynamically.
    """
...
class custom_nn_CircularPad2d(nn.Module):
    def __init__(self, L, R, T, B):
        super(custom_nn_CircularPad2d, self).__init__()
        self.L = L
        self.R = R
        self.T = T
        self.B = B
