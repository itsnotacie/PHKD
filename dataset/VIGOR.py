...
class VIGOR(Dataset):
    def __init__(self, args, split='train', root=None, same_area=True):
        if root is None:
            root = getattr(args, 'dataset_root', '/data/dataset/VIGOR')
    ...
    def __getitem__(self, idx):
        ...
        rotated_pano = np.roll(resized_pano, int(random_ori * resized_pano.shape[1]), axis=1)


        pano_bev = get_BEV_tensor(rotated_pano, 500, 500, dty=0, dy=0, out=self.out).numpy().astype(np.uint8)
        ...
        return bev, sat, pano_gps, sat_gps, ori_angle, torch.tensor(sat_delta, dtype=torch.float32), torch.tensor(
            ...
            resized_pano, rotated_pano, city
...
def fetch_dataloader(args, vigor=None, split='train'):
    ...
    if split == 'train':
        ...
        if getattr(args, "distributed", False):
            ...
            train_dataloader = DataLoader(training_set, batch_sampler=train_bs, num_workers=num_workers,
                                          persistent_workers=persistent)
        ...
        nw = min([os.cpu_count(), args.batch_size if args.batch_size > 1 else 0, getattr(args, 'num_workers', 8)])  # number of workers
        print('Using {} dataloader workers every process'.format(nw))
        test_sampler = DistributedSampler(vigor, shuffle=False, drop_last=False) if getattr(args, "distributed", False) else None
        test_loader = data.DataLoader(vigor, batch_size=args.batch_size,
                                      pin_memory=True, shuffle=False,
                                      sampler=test_sampler, num_workers=nw, drop_last=False)
        return test_loader
