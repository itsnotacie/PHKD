...
class DistanceBatchSampler:
    def __init__(self, sampler, batch_size, drop_last, required_dis, file_name, root=root_dir):
...
def load_train_data(args, batch_size, shift_range_lat=20, shift_range_lon=20, rotation_range=10):
    dataset_root = getattr(args, 'dataset_root', root_dir)
    split_file = getattr(args, 'train_file', train_file)
    ...
    train_set = SatGrdDataset(args=args, root=dataset_root, file=split_file,
    ...
    num_workers = getattr(args, "num_workers", num_thread_workers)
    if getattr(args, "distributed", False):
        ...
        bs = DistanceBatchSampler(torch.utils.data.RandomSampler(train_set), batch_size, True,
                                  2 * (max(shift_range_lat, shift_range_lon) * 1.5), file_name, root=dataset_root)
        train_loader = DataLoader(train_set, batch_sampler=bs, num_workers=num_workers)
...
def load_test1_data(batch_size, shift_range_lat=20, shift_range_lon=20, rotation_range=10):
    ...
    test1_set = SatGrdDatasetTest(root=getattr(load_test1_data, 'dataset_root', root_dir),
                                  file=getattr(load_test1_data, 'test1_file', test1_file),
                                  transform=(satmap_transform, grdimage_transform),
                                  shift_range_lat=shift_range_lat,
                                  shift_range_lon=shift_range_lon,
                                  rotation_range=rotation_range)

    test_sampler = DistributedSampler(test1_set, shuffle=False, drop_last=False) if getattr(load_test1_data, 'distributed', False) else None
    test1_loader = DataLoader(test1_set, batch_size=batch_size, shuffle=False, pin_memory=True,
                              sampler=test_sampler, num_workers=getattr(load_test1_data, 'num_workers', num_thread_workers), drop_last=False)
    return test1_loader
...
def load_test2_data(batch_size, shift_range_lat=20, shift_range_lon=20, rotation_range=10):
    ...
    test2_set = SatGrdDatasetTest(root=getattr(load_test2_data, 'dataset_root', root_dir),
                                  file=getattr(load_test2_data, 'test2_file', test2_file),
                                  transform=(satmap_transform, grdimage_transform),
                                  shift_range_lat=shift_range_lat,
                                  shift_range_lon=shift_range_lon,
                                  rotation_range=rotation_range)

    test_sampler = DistributedSampler(test2_set, shuffle=False, drop_last=False) if getattr(load_test2_data, 'distributed', False) else None
    test2_loader = DataLoader(test2_set, batch_size=batch_size, shuffle=False, pin_memory=True,
                              sampler=test_sampler, num_workers=getattr(load_test2_data, 'num_workers', num_thread_workers), drop_last=False)
    return test2_loader
